 ...
 *
 * Auto-confirm orders that have been delivered for 7+ days without customer
 * confirmation. Called daily by a cron service (e.g. Vercel Cron, GitHub Actions).
 *
 * Required environment variable:
 *   CRON_SECRET — a cryptographically random string (32+ chars).
 *   Example: openssl rand -hex 32
 *
 * The caller must send:
 *   Authorization: Bearer <CRON_SECRET>
 *
 * GET is intentionally NOT exported — cron must be POST only.
 */
export async function POST(request: NextRequest) {
  // ── AUTH GUARD ─────────────────────────────────────────────────────────────
  // Fail-closed: if CRON_SECRET is missing we treat the endpoint as locked.
  ...
  try {
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Failed to auto-confirm orders', details: updateError.message },
        { status: 500 }
      );
    }

    console.log(`[cron] Auto-confirmed ${updatedOrders?.length ?? 0} orders`);
